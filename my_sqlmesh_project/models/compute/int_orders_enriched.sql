MODEL (
  name models.int_orders_enriched,
  kind VIEW
);

WITH orders AS (
  SELECT * FROM models.stg_orders
),
items AS (
  SELECT order_id,
...
payments AS (
  ...
  FROM models.stg_order_payments
  GROUP BY 1
),
reviews AS (
  SELECT order_id,
         AVG(review_score) AS avg_review_score
  FROM models.stg_order_reviews
  GROUP BY 1
)
SELECT
  ...
  i.total_freight AS total_freight,
